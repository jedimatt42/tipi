...
unsigned char dsr_write(struct PAB* pab, char* record);
...
unsigned char dsr_write(struct PAB* pab, char* record) {
  ...
  vdpmemcpy(pab->VDPBuffer, (unsigned char*)record, len);
  pab->CharCount = len;
